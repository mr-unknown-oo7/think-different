...
//);
module updt_y_fsm(
input wire clk , rst_n , exe, empty,done_load_y,
input wire [4-1:0] y_dim_ip,
output wire done ,
output reg ptr_incr,load_y
);
reg [4-1:0] counter;
reg [3-1:0] next_state , state_reg;
wire done_temp;
assign done_temp = (counter == y_dim_ip && done_load_y)? 1'b1:1'b0;
assign done = done_temp;
assign done_temp_2 = (counter == y_dim_ip)? 1'b1:1'b0;
...
always @(*) begin  
     ...
     BUFF:next_state = (done_temp_2)?IDLE:BUFF;
...
//         valid <= (empty)?1'b0:1'b1;
        ...
        MID_EXE: begin
         counter <= (empty||done_temp_2)?counter:counter+4'b0001;
         ptr_incr <= (empty||done_temp_2)?1'b0:1'b1;
//         valid <= (empty||done_temp_2)?1'b0:1'b1;
         load_y <= 1'b0;
        end
